    ...
    class ProperPiperTTS {
        constructor(voiceId = "en_US-hfc_female-medium") {
            ...
            this.availableVoices = {
                ...
                "en_US-lessac-high": "US Male (Lessac) - High",
        ...
        async synthesize(text, speed = 1.0) {
            ...
            try {
                ...
                const inputs = {
                    input: new ort.Tensor("int64", new BigInt64Array(phonemeIds.map((id) => BigInt(id))), [
                        1,
                        phonemeIds.length,
                    ]),
                    input_lengths: new ort.Tensor("int64", new BigInt64Array([BigInt(phonemeIds.length)]), [1]),
                    scales: new ort.Tensor("float32", new Float32Array([0.667, speed, 0.8]), [3]),
                };
                ...
                try {
                    ...
                    if (typeof error === "number") {
                        ...
                        throw new Error(
                            `ONNX inference failed with code ${error}. This often means the session is corrupted or out of memory.`,
                        );
                    }
                    throw error;
                }
